...
class SegDataset(Dataset):
    def __init__(self, transform, data_path='./data/training', split_percent=0.85, validation=False, upscaling=None):
        if upscaling == "vdsr":
...
def get_train_val_dataloaders(split_percent=0.85, batch_size=4, num_workers=4, transform_train=None, transform_val=None, include_massachusetts=False, upscaling=None):
    train_dataset = SegDataset(transform_train, split_percent=split_percent, validation=False, upscaling=upscaling)
    val_dataset = SegDataset(transform_val, split_percent=split_percent, validation=True, upscaling=upscaling)
